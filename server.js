...
app.post("/post", (req, res) => {
  ...
  let newPost = {
    author: req.body.author || "",
    body: req.body.body || "",
    thread_id: req.body.thread_id || "", //could add an error right here.
  };
  Thread.findByIdAndUpdate(
    ...
    (err, thread) => {
      if (err != null) {
        ...
        res.status(404);
        console.log(
          "Thread does not exist. Can't post a thread to unexistent thread"
        );
      } else {
...
app.delete("/post/:thread_id/:post_id", (req, res) => {
  res.setHeader("Content-Type", "application/json");
  console.log(
    `deleting post of a thread ${req.params.thread_id} post ${req.params.post_id}`
  );
  Thread.findByIdAndUpdate(
